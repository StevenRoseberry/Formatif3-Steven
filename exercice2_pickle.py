...
class StationMesure:
    ...
    def to_dict(self):
        return {
            ...
            "capteurs": self.capteurs
        }

    @classmethod
    def from_dict(cls, data):
        station = cls(data["nom_station"])
        for capteur_data in data["capteurs"]:
            type_capteur = capteur_data["type"]
            capteur_cls = globals().get(type_capteur)
            if capteur_cls is None:
                raise ValueError(f"Type de valeur inconnu: {type_capteur}")
...
if __name__ == "__main__":
    ...
    with open("stations.plk", "wb") as f:
        pickle.dump(station, f)

# Si le write est binaire (wb) aussi faire un read binaire (rb)
    with open("stations.plk", "rb") as f:
        station_charge = pickle.load(f)

    # Affichage des descriptions
    StationMesure.to_dict()
    print(station_charge)
    ...
    for i in range(5):
        ...
        for capteur in station.capteurs:
            ...
            print(f"\tMesure de {capteur.__class__.__name__} ({capteur.id_capteur}) : {round(valeur, 2)} {capteur.unite} .")
